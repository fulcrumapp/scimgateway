...
const ScimGateway = function () {
  ...
  const auth = async (ctx, next) => { // authentication/authorization
    ...
    try { // authenticate
      const arrResolve = await Promise.all([
        ...
        .catch((err) => { throw (err) })
      for (const i in arrResolve) {
        logger.info("method " + arrResolve[i]);
      ...
      // all false - invalid auth method or missing plugin config
      let err
      ...
      logger.info("caught error, trying dumb methods " + err);
      const body = {}
      if (authType === 'Bearer') {
        let str = 'realm=""'
        if (err.token_error) {
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    try {
      const res = await this[handle.getMethod](ctx, utils.copyObj(getObj), ctx.query.attributes ? ctx.query.attributes.split(',').map(item => item.trim()) : [])
      ...
      if (handle.getMethod === handler.users.getMethod && Object.keys(userObj).length > 0 && !userObj.groups && !config.scim.groupMemberOfUser) { // groupMemberOfUser can be set to true for skipping
        ...
        if ((!ctx.query.attributes || arrAttr.includes('groups')) && typeof this[handler.groups.getMethod] === 'function') { // include groups
          ...
          let res
          try {
            res = await this[handler.groups.getMethod](ctx, { attribute: 'members.value', operator: 'eq', value: getObj.value }, ['members.value', 'id', 'displayName'])
      ...
      if (handle.getMethod === handler.users.getMethod && !config.scim.groupMemberOfUser) { // groupMemberOfUser can be set to true for skipping
        ...
        if ((!ctx.query.attributes || arrAttr.includes('groups')) && typeof this[handler.groups.getMethod] === 'function') { // include groups
          for (let j = 0; j < scimdata.Resources.length; j++) {
            ...
            let res
            try {
              res = await this[handler.groups.getMethod](ctx, { attribute: 'members.value', operator: 'eq', value: decodeURIComponent(userObj.id) }, ['members.value', 'id', 'displayName']) // await scimgateway.getUserGroups(baseEntity, userObj.id, 'members.value,displayName')
            } catch (err) {} // method may be implemented but throwing error like groups not supported/implemented
            if (res && res.Resources && Array.isArray(res.Resources) && res.Resources.length > 0) {
              userObj.groups = []
              for (let i = 0; i < res.Resources.length; i++) {
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`], async (ctx) => {
    ...
    try {
      const res = await this[handle.createMethod](ctx, scimdata)
        ...
        try {
          if (handle.createMethod === 'createUser') {
            if (jsonBody.userName) {
              ...
              obj = await this[handle.getMethod](ctx, { attribute: 'userName', operator: 'eq', value: jsonBody.userName }, ['id', 'userName'])
            } else if (jsonBody.externalId) {
              jsonBody.id = jsonBody.externalId
              obj = await this[handle.getMethod](ctx, { attribute: 'externalId', operator: 'eq', value: jsonBody.externalId }, ['id', 'externalId'])
            }
            ...
            if (jsonBody.externalId) {
              ...
              obj = await this[handle.getMethod](ctx, { attribute: 'displayName', operator: 'eq', value: jsonBody.displayName }, ['id', 'displayName'])
            }
          }
        } catch (err) { }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups)/:id`], async (ctx) => {
    ...
    try {
      await this[handle.deleteMethod](ctx, id)
      ctx.status = 204
    } catch (err) {
      ctx.status = 500
      const e = jsonErr(config.scim.version, pluginName, ctx.status, err)
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        await this[handle.modifyMethod](ctx, id, scimdata)
        if (ctx.query.attributes || ctx.query.excludedAttributes) {
          logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)

          const res = await this[handle.getMethod](ctx, { attribute: 'id', operator: 'eq', value: id }, ctx.query.attributes ? ctx.query.attributes.split(',').map(item => item.trim()) : [])
          let scimdata = {
            Resources: [],
            totalResults: null
          }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        // get current object
        logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
        let res = await this[handle.getMethod](ctx, { attribute: 'id', operator: 'eq', value: id }, [])
        ...
        if (jsonBody.groups && Array.isArray(jsonBody.groups)) { // only if groups included, { "groups": [] } will remove all existing
          ...
          if (currentObj.groups && Array.isArray(currentObj.groups)) currentGroups = currentObj.groups
            ...
            let res
            try {
              res = await this[handler.groups.getMethod](ctx, { attribute: 'members.value', operator: 'eq', value: decodeURIComponent(id) }, ['members.value', 'id', 'displayName']) // await scimgateway.getUserGroups(baseEntity, userObj.id, 'members.value,displayName')
            } catch (err) {} // method may be implemented but throwing error like groups not supported/implemented
            currentGroups = []
            if (res && res.Resources && Array.isArray(res.Resources) && res.Resources.length > 0) {
              for (let i = 0; i < res.Resources.length; i++) {
          ...
          const addGroups = async (grp) => {
            ...
            return await this[handler.groups.modifyMethod](ctx, grp, obj)
          }
        ...
        // get updated object
        logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
        res = await this[handle.getMethod](ctx, { attribute: 'id', operator: 'eq', value: id }, [])
        ...
        if (handle.getMethod === handler.users.getMethod && typeof this[handler.groups.getMethod] === 'function') {
          ...
          const res = await this[handler.groups.getMethod](ctx, { attribute: 'members.value', operator: 'eq', value: id }, ['members.value', 'id', 'displayName'])
          let grps = []
  ...
  router.post(['/api', '/:baseEntity/api'], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        let result = await this.postApi(ctx, apiObj)
        const location = ctx.origin + ctx.path
        if (result) {
  ...
  router.put(['/api/:id', '/:baseEntity/api/:id'], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        let result = await this.putApi(ctx, id, apiObj)
        const location = ctx.origin + ctx.path
        if (result) {
          if (typeof result === 'object') result = { result: result }
          else {
  ...
  router.patch(['/api/:id', '/:baseEntity/api/:id'], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        let result = await this.patchApi(ctx, id, apiObj)
        const location = ctx.origin + ctx.path
        if (result) {
          if (typeof result === 'object') result = { result: result }
          else {
  ...
  router.get(['/api', '/api/:id',
    ...
    try {
      let result = await this.getApi(ctx, ctx.params.id, ctx.query, apiObj)
      const location = ctx.origin + ctx.path
      if (result) {
        if (typeof result === 'object') result = { result: result }
        else {
  ...
  router.delete(['/api/:id', '/:baseEntity/api/:id'], async (ctx) => {
    ...
    try {
      let result = await this.deleteApi(ctx, id)
      if (result) {
        if (typeof result === 'object') result = { result: result }
        else {
          try {
